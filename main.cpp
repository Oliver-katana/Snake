...
{
	bool left_arrow = GetKeyState(0x25) & 0x8000;//äâèæåíèå âëåâî
	bool up_arrow = GetKeyState(0x26) & 0x8000;//äâèæåíèå ââåðõ
	bool right_arrow = GetKeyState(0x27) & 0x8000;//äâèæåíèå âïðàâî
	bool down_arrow = GetKeyState(0x28) & 0x8000;//äâèæåíèå âíèç

	bool esc = GetKeyState(0x1B) & 0x8000;//âûõîä èç èãðû

	if (esc) { exit(0); }
	if (left_arrow) { move = move_left; }
	if (up_arrow) { move = move_up; }
	if (right_arrow) { move = move_right; }
	if (down_arrow) { move = move_down; }
};
